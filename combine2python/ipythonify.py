...
import sys, os, errno, time, json, shutil, re
...
def codestitch(pymodelloc, extloc, filename):
    ...
    sedmlstro = sedmlconv(extloc)
    sedmlstr, tempPath = codeanalysis(sedmlstro, outputstr)
    outputstr = outputstr + tempPath[1][1] + " = '''\n" + sbmlstr + "'''\n" + sedmlstr
    with open(pymodelloc, "w+") as filef:
        filef.seek(0)
        #readf = filef.read()
        filef.write(outputstr)
        filef.close()


#Included in case of SEDML codes not compatible with single model file approach
def codeanalysis(sedmlstr, outputstr):
    tempPath = [[]]
